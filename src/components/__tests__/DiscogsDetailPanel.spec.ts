...
describe('DiscogsDetailPanel', () => {
  ...
  it('splits "Artist - Title" into separate artist and title', () => {
    const wrapper = mount(DiscogsDetailPanel, { props: { result: baseResult } })
    expect(wrapper.find('.detail-title').text()).toBe('Nevermind')
    expect(wrapper.find('.detail-artist').text()).toBe('Nirvana')
  })
  ...
  it('renders the full title with no artist line when there is no " - " separator', () => {
    ...
    expect(wrapper.find('.detail-title').text()).toBe('Nevermind')
    expect(wrapper.find('.detail-artist').exists()).toBe(false)
  })
  ...
  it('renders genre and style values as tags', () => {
    ...
    const tags = wrapper.findAll('.tag').map((tag) => tag.text())
